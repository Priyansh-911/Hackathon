import {
  Box,
  Button,
  Card,
  CardContent,
  CardMedia,
  CardActions,
  Container,
  Grid,
  Typography,
  TextField,
} from "@mui/material";
import React, { useEffect, useState } from "react";
import AccountCircleIcon from "@mui/icons-material/AccountCircle";
import { useNavigate } from "react-router-dom";
import HelpOutlineIcon from "@mui/icons-material/HelpOutline";
import Footer from "../component/footer";
import Link from '@mui/material/Link';

function Student() {
  const [comment, setComment] = useState("");

  const data = async () => {
    console.warn(comment);
    let result = await fetch("http://localhost:4500/dashboard/student", {
      method: "post",
      body: JSON.stringify({comment}),
      headers: {
        "Content-Type": "application/json",
      },
    });
    result = await result.json();
    console.warn(result);
  };

  let navigate = useNavigate();
  const btstyle={width:"30%"};
  const btnstyle = { margin: "10px 0", width:"10%" };
  const [scholarships, setscholarships] = useState("");

  /*useEffect(()=>{
    const fetchdata = async () => {
      const data= await axios.get("/scholarships/get");
      setscholarships(data);
    };
    fetchdata();
  },[])*/

  return (
    <main>
      <div>
        <Box padding={5}>
          <Button
            style={btstyle}
            color="secondary"
            size="large"
            onClick={() => {
              navigate("/dashboard/student/info");
            }}
          >
            <AccountCircleIcon fontSize="large" />
            <Typography variant="h4">Welcome back!!</Typography>
          </Button>
        </Box>


       
            <h1>Your Scholarships</h1>
            <Grid
            container spacing={6}
           justifyContent={"center"}
           marginBottom={6}
            // sx={{margin: '0px 0px 0px '}}
            >
             <Grid item xs={4}>
             <Card sx={{ minWidth: 275}}>
      <CardContent>
        <img src="https://www.webexam.in/wp-content/uploads/2018/08/IndusInd-Scholarship.jpg"  alt="indu" style={{ width: "100%", height: "40vh" }}></img>


        
       
      
     
        <Link href="http://www.theindusindfoundation.org/"> INDUSIND FOUNDATION SCHOLARSHIP</Link>




</CardContent>




 



    </Card>
              </Grid> 
              <Grid item xs={4} >
             <Card sx={{ minWidth: 265 }}>
      <CardContent>
        <img src="https://i0.wp.com/www.scholarshipsinindia.com/wp-content/uploads/Siemens-Scholarship-Program-2.jpg?fit=200%2C200&ssl=1" alt="indu" style={{ width: "100%", height: "40vh" }}></img>
        
         
          <Link href="https://press.siemens.com/in/en/pressrelease/siemens-scholarship-program-2021-2022-opens-applications">  Seimens Scholarship Program</Link>

        
      </CardContent>
    </Card>
              </Grid> 
              <Grid item xs={4}>
             <Card sx={{ minWidth: 275 }}>
      <CardContent>
      
      <img src="https://www.careerindia.com/img/1200x60x675/2021/08/aicte2-1630315223.jpg" alt="indu" style={{ width: "100%", height: "40vh" }}></img>
        <Link href="https://www.aicte-pragati-saksham-gov.in/"> AICTE Saksham Scholarship</Link>
      </CardContent>
    </Card>



              </Grid> 
            </Grid>
           
     
      
       
      <Grid
            container spacing={6}
            marginBottom={6}
           // justifyContent={"center"}
            // sx={{margin: '1px 2px 10px '}}
            >
             <Grid item xs={4}>
             <Card sx={{ minWidth: 275 }}>
      <CardContent>
        <img src="data:image/jpeg;base64," alt="indu" style={{ width: "100%", height: "40vh" }}></img>
        {/* <Typography sx={{ fontSize: 14 }} color="text.secondary" gutterBottom>
          Gandhi Fellowship Program
        </Typography> */}
        <Link href="https://gandhifellowship.org/">   Gandhi Fellowship Program</Link>
        











       
      </CardContent>
    </Card>
              </Grid> 
              <Grid item xs={4}>
             <Card sx={{ minWidth: 275 }}>
      <CardContent>
        <img src="https://buddy4study.s3.ap-southeast-1.amazonaws.com/article/wp-content/uploads/2022/08/06155611/Central-Sector-Scholarship-%E2%80%93-A-Scholarship-Scheme-for-College-and-University-Students.jpg" alt="indu" style={{ width: "100%", height: "40vh" }}></img>
        
        <Link href="https://central-scholarship.org/">  Central Sector Scholarship </Link>
      </CardContent>
    </Card>
              </Grid> 
              <Grid item xs={4}>
             <Card sx={{ minWidth: 275 }}>
      <CardContent>
        <img src="https://buddy4study.s3.ap-southeast-1.amazonaws.com/article/wp-content/uploads/2021/09/29142432/Pragati-Scholarship-%E2%80%93-AICTE-Scholarship-Scheme-for-Girl-Child.jpg" alt="indu" style={{ width: "100%", height: "40vh" }}></img>
      
        <Link href="https://www.aicte-pragati-saksham-gov.in/">   AICTE Pragati Scholarship</Link>
        
       
      </CardContent>
    </Card>
        


    
    
    </Grid>
    </Grid>
   




     
        
       
      <Grid
            container spacing={6}
           // justifyContent={"center"}
            // sx={{margin: '1px 2px 10px '}}
            >
             <Grid item xs={4}>
             <Card sx={{ minWidth: 275 }}>
      <CardContent>
        <img src="https://leverageedublog.s3.ap-south-1.amazonaws.com/blog/wp-content/uploads/2020/10/06202610/UGC-Scholarships-for-girls.jpg" alt="indu" style={{ width: "100%", height: "40vh" }}></img>
        
         
         <Link href="https://www.ugc.ac.in/page/scholarships-and-fellowships.aspx">  UGC Grant Scholarship</Link>
       
      </CardContent>
    </Card>
              </Grid> 
              <Grid item xs={4}>
             <Card sx={{ minWidth: 275 }}>
      <CardContent>
        <img src="https://indiacsr.in/wp-content/uploads/2022/07/kc-mahindra.jpg" alt="indu" style={{ width: "100%", height: "40vh" }}></img>
        {/* <Typography sx={{ fontSize: 14 }} color="text.secondary" gutterBottom>
          K.C.MAHINDRA Scholar
          ship
        </Typography> */}

<Link href="https://www.kcmet.org/what-we-do-Scholarship-Grants.aspx">   K.C.MAHINDRA Scholar</Link>
      </CardContent>
    </Card>
              </Grid> 
              <Grid item xs={4}>
             <Card sx={{ minWidth: 275 }}>
      <CardContent>
        <img src="https://career.webindia123.com/career/options/images/aditybirla.jpg" alt="indu" style={{ width: "100%", height: "40vh" }}></img>
        {/* <Typography sx={{ fontSize: 14 }} color="text.secondary" gutterBottom>
          INDU MY DARLING
        </Typography> */}
        <Link href="https://www.adityabirlascholars.net/the-scholarship/">  Aditya Birla Scholarships</Link>
        
       
      </CardContent>
    </Card>
        
    
    
    </Grid>
    </Grid>
   



        <Box padding={10} margin={5}>
          <Typography variant="h4">
            <HelpOutlineIcon />
            Ask for help
          </Typography>
          <TextField
            id="outlined-multiline-static"
            label="Type here"
            multiline
            rows={5}
            value={comment}
            onChange={(e) => setComment(e.target.value)}
            margin="dense"
            fullWidth
          />
          <Button
            type="submit"
            color="info"
            variant="contained"
            style={btnstyle}
            onClick={data}
          >
            Submit
          </Button>
        </Box>
        <Footer />
      </div>
    </main>
  );
}
/*<Container maxWidth="md">
             <Grid container spacing={4}>
              <Grid item>
                {
                  scholarships && scholarships?.data.map((scholarship)=>(
                  <Card id={scholarship._id}
                  sx={{ maxWidth: 345 }}>
                    <CardMedia
                      
                      sx={{ height: 140 }}
                      image={scholarship.imageURL}
                      
                    />
                    <CardContent title={scholarship.title}>
                      <Typography gutterBottom variant="h5" component="div">
                        title
                      </Typography>
                      <Typography variant="body2" color="text.secondary">
                        It is applicable for 12th students of uttar pradesh.
                      </Typography>
                    </CardContent>
                    <CardActions>
                      <Button size="small">Share</Button>
                      <Button size="small">Learn More</Button>
                    </CardActions>
                  </Card>
                  ))
                }
              
              </Grid>
             </Grid>
        </Container>*/

export default Student;
